function MomentoX(u_old,v_old,u,v,P,nIT_vel)
    ...
    for i in 3:nx,j in 2:ny
       ...
       mn = .5*ρ*dx*(v_old[i-1,j+1]+v_old[i,j+1])
       ms = .5*ρ*dx*(v_old[i-1,j]+v_old[i,j])
        Ae[i,j] = maximum([-me,0])
        Aw[i,j] = maximum([mw,0])
        An[i,j] = maximum([-mn,0])
        As[i,j] = maximum([ms,0])
...
function MomentoY(u_old,v_old,u,v,P,nIT_vel)
    ...
    for i in 2:nx,j in 3:ny
       me = .5*ρ*dy*(u_old[i+1,j]+u_old[i+1,j-1])
       mw = .5*ρ*dy*(u_old[i,j-1]+u_old[i,j])
       mn = .5*ρ*dx*(v_old[i,j]+v_old[i,j+1])
       ms = .5*ρ*dx*(v_old[i,j-1]+v_old[i,j])
        Ae[i,j] = maximum([-me,0])
        Aw[i,j] = maximum([mw,0])
        An[i,j] = maximum([-mn,0])
        As[i,j] = maximum([ms,0])
