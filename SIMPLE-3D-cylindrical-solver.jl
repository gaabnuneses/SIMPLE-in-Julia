...
function MomentoX(u_old,v_old,w_old,u,v,w,P,nIT_vel)
    ...
    for i in 3:nx,j in 2:ny,k in 2:nz
        ...
        Ae[i,j,k] += μ*ae/dx
        Aw[i,j,k] += μ*aw/dx
        An[i,j,k] += μ*an/(dy*x[i])
        As[i,j,k] += μ*as/(dy*x[i])
        At[i,j,k] += μ*at/dz
        Ab[i,j,k] += μ*ab/dz
    end
    ...
    for i in 3:nx, k in 2:nz
        an = (x[i+1]-x[i+1])/2*(z[k]+z[k+1])/2
        as = (x[i+1]-x[i+1])/2*(z[k]+z[k+1])/2
        mn = .5*ρ*ae*(v_old[i-1,ny+1,k]+v_old[i,ny+1,k])
        ms = .5*ρ*as*(v_old[i-1,2,k]+v_old[i,2,k])
        An[i,ny,k] = maximum([-mn,0])+ μ*an/(dy*x[i])
        As[i,2,k] = maximum([ms,0])+ μ*as/(dy*x[i])
    end

    for i in 3:nx, j in 2:ny
        at = (y[j+1]-y[j])*(((x[i]+x[i+1])/2)^2-((x[i]+x[i-1])/2)^2)/2
        ab = (y[j+1]-y[j])*(((x[i]+x[i+1])/2)^2-((x[i]+x[i-1])/2)^2)/2
        At[i,j,nz] = maximum([-mt,0])+ μ*at/dz
        Ab[i,j,2] = maximum([mb,0])+ μ*ab/dz
    end
    ...
    for nit = 1:nIT_vel
        for i = 3:nx
            for j = 2:ny
                for k = 2:nz
                    dV = -(y[j-1]-y[j+1])*(z[k-1]-z[k+1])*(2*x[i]+x[i-1+r[i+1]])*(x[i-1]-x[i+1])/32
                    dX = x[i]-x[i-1]
